...
const getTokenAndSave = async () => {
  while (true) {
    // Loop sampai berhasil mendapatkan token yang valid
    try {
      const token = await getToken();
      fs.writeFileSync(TOKEN_FILE_PATH, token);
      console.log("✅ New token has been saved.");
      return token; // Keluar dari loop jika berhasil
    } catch (error) {
      if (
        error.response &&
        (error.response.status === 401 || error.response.status === 520)
      ) {
        console.error(
          `🚨 Error ${error.response.status} occurred while getting token. Retrying in 3 seconds...`
            .red
        );
        await delay(3000); // Tunda sejenak sebelum mencoba lagi
      } else {
        throw error; // Melempar kembali error lain untuk ditangani di tempat lain
      }
    }
  }
};
...
const handleOneTimeFlow = async (token) => {
  try {
    console.log(`Starting one-time flow... - [${new Date().toLocaleString()}]`);
    ...
    if (
      ...
      console.error(
        `🚨 Error ${error.response.status} occurred. Refreshing token and retrying one-time flow...`
          .red
      );
      await delay(3000); // Tunda sejenak sebelum mencoba lagi
      const newToken = await getTokenAndSave();
      await handleOneTimeFlow(newToken); // Jalankan ulang handleOneTimeFlow dengan token baru
    } else if (
      error.response &&
      error.response.data &&
...
const runScript = async () => {
  ...
  if (option === "1") {
    ...
    while (true) {
      ...
      // Generate random interval between 3 and 4 hours in milliseconds
      const randomInterval =
        Math.floor(Math.random() * (3600000 * 1)) + 3600000 * 3;

      console.log(
        `🔄 Restarting one-time flow in ${randomInterval / 3600000} hours...`
          .yellow
      );
